(function(){
  var d = document;
  var w = window;
  var script = d.currentScript;
  var cfg = {
    publicKey: script?.dataset.ollehPublicKey || "",
    baseUrl: script?.dataset.ollehBaseUrl || "https://olleh.ai/demo",
    lang: script?.dataset.ollehLang || "en",
    autostart: String(script?.dataset.ollehAutostart || "false") === "true",
    allow: "microphone; camera; autoplay",
    sandbox: "allow-scripts allow-forms allow-same-origin allow-presentation"
  };

  var api = { open: openModal, close: closeModal, toggle: toggleModal, on: on };
  var listeners = { open: [], close: [] };
  function on(evt, cb){ if(listeners[evt]) listeners[evt].push(cb); }
  function emit(evt){ (listeners[evt]||[]).forEach(function(cb){ try{ cb(); }catch(e){} }); }

  // public API queue
  w.OllehAIQueue = w.OllehAIQueue || [];
  w.OllehAI = api;
  w.OllehAIQueue.forEach(function(fn){ try{ fn(api); }catch(e){} });
  w.OllehAIQueue.push = function(fn){ try{ fn(api); }catch(e){} };

  // elements
  var root = d.createElement('div');
  root.id = 'olleh-ai-root';
  root.style.position = 'fixed';
  root.style.zIndex = '2147483000';
  d.body.appendChild(root);

  var btn = d.createElement('button');
  btn.setAttribute('type', 'button');
  btn.setAttribute('aria-label', 'Open Olleh AI Assistant');
  btn.style.position = 'fixed';
  btn.style.right = '24px';
  btn.style.bottom = '24px';
  btn.style.width = '56px';
  btn.style.height = '56px';
  btn.style.borderRadius = '9999px';
  btn.style.border = '0';
  btn.style.cursor = 'pointer';
  btn.style.color = '#fff';
  btn.style.boxShadow = '0 8px 24px rgba(0,0,0,0.25)';
  btn.style.background = 'linear-gradient(90deg, #9333ea, #3b82f6)';
  btn.style.display = 'flex';
  btn.style.alignItems = 'center';
  btn.style.justifyContent = 'center';
  btn.style.transition = 'transform 120ms ease';
  btn.onpointerdown = function(){ btn.style.transform = 'scale(1.06) rotate(6deg)'; };
  btn.onpointerup = function(){ btn.style.transform = 'scale(1)'; };
  btn.onclick = toggleModal;
  btn.innerHTML = '<svg viewBox="0 0 640 640" width="28" height="28" fill="currentColor" aria-hidden="true"><path d="M320 64C267 64 224 107 224 160L224 288C224 341 267 384 320 384C373 384 416 341 416 288L416 160C416 107 373 64 320 64zM176 248C176 234.7 165.3 224 152 224C138.7 224 128 234.7 128 248L128 288C128 385.9 201.3 466.7 296 478.5L296 528L248 528C234.7 528 224 538.7 224 552C224 565.3 234.7 576 248 576L392 576C405.3 576 416 565.3 416 552C416 538.7 405.3 528 392 528L344 528L344 478.5C438.7 466.7 512 385.9 512 288L512 248C512 234.7 501.3 224 488 224C474.7 224 464 234.7 464 248L464 288C464 367.5 399.5 432 320 432C240.5 432 176 367.5 176 288L176 248z"/></svg>';
  d.body.appendChild(btn);

  var scrim = d.createElement('div');
  scrim.style.position = 'fixed';
  scrim.style.inset = '0';
  scrim.style.background = 'rgba(0,0,0,0.25)';
  scrim.style.opacity = '0';
  scrim.style.transition = 'opacity 200ms ease';
  scrim.style.pointerEvents = 'none';
  scrim.style.zIndex = '2147482999';
  d.body.appendChild(scrim);

  var modal = d.createElement('div');
  modal.setAttribute('role', 'dialog');
  modal.setAttribute('aria-modal', 'true');
  modal.setAttribute('aria-label', 'Olleh AI Assistant');
  modal.style.position = 'fixed';
  modal.style.right = '16px';
  modal.style.bottom = '96px';
  modal.style.width = '26rem';
  modal.style.maxWidth = 'calc(100vw - 24px)';
  modal.style.maxHeight = '80vh';
  modal.style.background = '#fff';
  modal.style.borderRadius = '16px';
  modal.style.boxShadow = '0 20px 50px rgba(0,0,0,0.25)';
  modal.style.border = '1px solid #e5e7eb';
  modal.style.overflow = 'hidden';
  modal.style.transform = 'translateY(24px)';
  modal.style.opacity = '0';
  modal.style.transition = 'transform 200ms ease, opacity 200ms ease';
  modal.style.zIndex = '2147483000';
  modal.tabIndex = -1;
  d.body.appendChild(modal);

  var header = d.createElement('div');
  header.style.display = 'flex';
  header.style.alignItems = 'center';
  header.style.justifyContent = 'space-between';
  header.style.padding = '10px 12px';
  header.style.background = 'linear-gradient(90deg,#22c55e,#10b981)';
  header.style.color = '#fff';
  var title = d.createElement('span');
  title.textContent = 'Olleh AI Assistant';
  var closeBtn = d.createElement('button');
  closeBtn.type = 'button';
  closeBtn.setAttribute('aria-label', 'Close');
  closeBtn.style.padding = '6px';
  closeBtn.style.border = '0';
  closeBtn.style.background = 'transparent';
  closeBtn.style.color = '#fff';
  closeBtn.style.cursor = 'pointer';
  closeBtn.innerHTML = '<svg viewBox="0 0 640 640" width="20" height="20" fill="currentColor" aria-hidden="true"><path d="M504.6 148.5C515.9 134.9 514.1 114.7 500.5 103.4C486.9 92.1 466.7 93.9 455.4 107.5L320 270L184.6 107.5C173.3 93.9 153.1 92.1 139.5 103.4C125.9 114.7 124.1 134.9 135.4 148.5L278.3 320L135.4 491.5C124.1 505.1 125.9 525.3 139.5 536.6C153.1 547.9 173.3 546.1 184.6 532.5L320 370L455.4 532.5C466.7 546.1 486.9 547.9 500.5 536.6C514.1 525.3 515.9 505.1 504.6 491.5L361.7 320L504.6 148.5z"/></svg>';
  closeBtn.onclick = closeModal;
  header.appendChild(title);
  header.appendChild(closeBtn);
  modal.appendChild(header);

  var frameWrap = d.createElement('div');
  frameWrap.style.position = 'relative';
  frameWrap.style.width = '100%';
  frameWrap.style.height = '65vh';
  frameWrap.style.maxHeight = 'calc(80vh - 44px)';
  modal.appendChild(frameWrap);

  var iframe = d.createElement('iframe');
  iframe.title = 'Olleh AI';
  iframe.style.width = '100%';
  iframe.style.height = '100%';
  iframe.style.border = '0';
  iframe.allow = cfg.allow;
  iframe.sandbox = cfg.sandbox;
  frameWrap.appendChild(iframe);

  var isOpen = false;
  var lastActive = null;

  function openModal(){
    if(isOpen) return;
    isOpen = true;
    lastActive = d.activeElement;
    btn.setAttribute('aria-label', 'Close Olleh AI Assistant');
    scrim.style.pointerEvents = 'auto';
    scrim.style.opacity = '1';
    modal.style.opacity = '1';
    modal.style.transform = 'translateY(0)';
    d.body.style.overflow = 'hidden';
    // token fetch then set src
    getToken().then(function(token){
      var url = cfg.baseUrl + (cfg.baseUrl.indexOf('?')>-1 ? '&' : '?') + 'token=' + encodeURIComponent(token) + '&lang=' + encodeURIComponent(cfg.lang);
      iframe.src = url;
      tryFocus(modal);
      emit('open');
    }).catch(function(){
      // fallback, no token, still load base page
      iframe.src = cfg.baseUrl;
      tryFocus(modal);
      emit('open');
    });
  }
  function closeModal(){
    if(!isOpen) return;
    isOpen = false;
    btn.setAttribute('aria-label', 'Open Olleh AI Assistant');
    scrim.style.opacity = '0';
    scrim.style.pointerEvents = 'none';
    modal.style.opacity = '0';
    modal.style.transform = 'translateY(24px)';
    d.body.style.overflow = '';
    // optional, stop media inside iframe by clearing src, comment if you want persistence
    // iframe.src = 'about:blank';
    if(lastActive && lastActive.focus) try{ lastActive.focus(); }catch(e){}
    emit('close');
  }
  function toggleModal(){ isOpen ? closeModal() : openModal(); }
  function tryFocus(el){ setTimeout(function(){ try{ el.focus(); }catch(e){} }, 0); }

  // outside click, Escape
  d.addEventListener('mousedown', function(e){
    if(!isOpen) return;
    if(!modal.contains(e.target) && !btn.contains(e.target)) closeModal();
  });
  d.addEventListener('keydown', function(e){
    if(!isOpen) return;
    if(e.key === 'Escape') closeModal();
  });

  // token fetch helper
  function getToken(){
    // prefer signed short lived token from your origin
    // customer sites call your endpoint, you return token based on cfg.publicKey
    var endpoint = 'https://api.yourdomain.com/olleh/embed-token';
    return fetch(endpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ publicKey: cfg.publicKey, lang: cfg.lang, origin: location.origin })
    }).then(function(r){
      if(!r.ok) throw new Error('token failed');
      return r.json();
    }).then(function(j){
      if(!j || !j.token) throw new Error('bad payload');
      return j.token;
    });
  }

  // optional autostart
  if(cfg.autostart) {
    if(document.readyState === 'complete' || document.readyState === 'interactive') openModal();
    else d.addEventListener('DOMContentLoaded', openModal);
  }
})();
